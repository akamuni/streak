...
interface Segment { start: Date; end: Date; length: number }
    ...
    const updateSegments = () => {
      ...
      sortedDates.forEach(dateObj => {
        ...
        if (!dateSet.has(prev)) {
          ...
          segs.push({ start: dateObj, end: new Date(current.getTime() - 86400000), length: len })
        }
      })
      ...
      <TableContainer component={Paper}>
        <Table size="small">
          ...
          <TableBody>
            {segments.map((s, idx) => (
              <TableRow key={idx}>
                <TableCell>{s.start.toDateString()}</TableCell>
                <TableCell>{s.end.toDateString()}</TableCell>
                <TableCell align="right">{s.length}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
